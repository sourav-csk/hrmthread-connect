...
interface LeaveRow {
  id: string; leave_type: LeaveType; start_date: string; end_date: string;
  reason: string; status: LeaveStatus; reviewer_notes: string | null; created_at: string;
}
...
const STATUS_ICONS: Record<LeaveStatus, typeof Clock> = { pending: Clock, approved: CheckCircle2, rejected: XCircle };
const LEAVE_LABELS: Record<LeaveType, string> = { casual: "Casual Leave", sick: "Sick Leave", earned: "Earned Leave", unpaid: "Unpaid Leave" };
...
export default function Leaves() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setLeaves((l ?? []) as LeaveRow[]); setBalance(e?.leave_balance ?? 0); setLoading(false);
    })();
  }, [user]);

  const resetForm = () => { setLeaveType("casual"); setStartDate(""); setEndDate(""); setReason(""); };
  ...
  const handleSubmit = async () => {
    ...
    if (leaveType !== "unpaid" && days > balance) { toast.error(`Insufficient balance (${balance} days remaining)`); return; }
    setSubmitting(true);
    const { data, error } = await supabase.from("leaves").insert({ user_id: user.id, leave_type: leaveType, start_date: startDate, end_date: endDate, reason: reason.trim() }).select().single();
    if (error) toast.error(error.message);
    else { setLeaves((prev) => [data as LeaveRow, ...prev]); toast.success("Leave application submitted"); resetForm(); setOpen(false); }
    setSubmitting(false);
  };
  ...
  return (
    <div className="px-4 xs:px-5 pt-5 xs:pt-6 space-y-4 pb-4">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-lg xs:text-xl font-bold tracking-tight">Leaves</h1>
          <p className="text-xs text-muted-foreground mt-0.5">Apply & track your leaves</p>
        </div>
          ...
          <DialogContent className="sm:max-w-md bg-card border-border">
            ...
            <div className="space-y-4 mt-2">
                ...
                <Select value={leaveType} onValueChange={(v) => setLeaveType(v as LeaveType)}>
                  ...
                  <SelectContent>
                    {(["casual", "sick", "earned", "unpaid"] as LeaveType[]).map((t) => (
                      <SelectItem key={t} value={t}>{LEAVE_LABELS[t]}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              ...
              {startDate && endDate && endDate >= startDate && (
                <p className="text-xs text-muted-foreground">Duration: <span className="text-foreground font-medium">{differenceInCalendarDays(parseISO(endDate), parseISO(startDate)) + 1} day(s)</span></p>
              )}
              <div className="space-y-1.5"><Label>Reason</Label><Textarea value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Brief reason…" rows={3} /></div>
      ...
      <div className="rounded-xl bg-card border border-border p-4 shadow-elevated">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-2xl font-bold tabular">{balance}<span className="text-xs font-medium text-muted-foreground ml-1">days</span></p>
          </div>
        </div>
        <div className="grid grid-cols-3 gap-3 mt-3 pt-3 border-t border-border">
          <div className="text-center">
            <p className="text-base font-bold tabular text-success">{leaves.filter((l) => l.status === "approved").length}</p>
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Approved</p>
          </div>
          <div className="text-center">
            <p className="text-base font-bold tabular text-warning">{pending.length}</p>
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Pending</p>
          </div>
          <div className="text-center">
            <p className="text-base font-bold tabular text-destructive">{leaves.filter((l) => l.status === "rejected").length}</p>
      ...
      <Tabs defaultValue="pending" className="w-full">
        ...
        <TabsContent value="pending" className="space-y-2 mt-3">
          {pending.length === 0 && <p className="text-xs text-muted-foreground text-center py-6">No pending applications</p>}
          {pending.map((l) => <LeaveCard key={l.id} leave={l} />)}
        </TabsContent>
        <TabsContent value="past" className="space-y-2 mt-3">
          {past.length === 0 && <p className="text-xs text-muted-foreground text-center py-6">No past records</p>}
  ...
  return (
    <div className="rounded-lg bg-card border border-border p-3.5 space-y-2 shadow-elevated">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-sm font-medium">{LEAVE_LABELS[leave.leave_type]}</p>
        ...
        <Badge variant="outline" className={`text-[10px] uppercase tracking-wider font-medium ${STATUS_COLORS[leave.status]}`}>
          <Icon className="h-3 w-3 mr-1" />{leave.status}
        </Badge>
      </div>
